...
// [GET] Get average electricity usage (1m) from DB
router.get('/:alat_id/usages/detail', async (req, res) => {
  try {
    const { alat_id } = req.params;
    const { date } = req.query;
    const usages = []
    ...
    Promise.all(result).then((usageResult => {
      // const groupedUsageById = Object.entries(
      //   usageResult.reduce((acc, { timestamp, totalPower, currents }) => {
      //     // Group initialization
      //     if (!acc[timestamp]) {
      //       acc[timestamp] = [];
      //     }
      //     // Grouping
      //     acc[timestamp].push(totalPower, currents);
      //     return acc;
      //   }, {})
      // ).map(([timestamp, totalPower]) => {
      //   const avgPower = totalPower.reduce((a, b) => a + b, 0) / totalPower.length;
